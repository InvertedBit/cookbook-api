...
exports.new = function (req, res) {
    ...
    if (req.files !== undefined && req.files.image !== undefined) {
        let imageFile = req.files.image;
            ...
            if (!fs.existsSync(basePath + staticDir)) {
                fs.mkdirSync(basePath + staticDir, { recursive: true });
            }
            fs.writeFileSync(basePath + staticDir + newPath, imageData);
    ...
    image.save(function (err) {
        if (err) {
            if (fs.existsSync(basePath + staticDir + image.path)) {
                fs.rmSync(basePath + staticDir + image.path);
            }
            console.log(err);
            res.json({
                status: "error",
